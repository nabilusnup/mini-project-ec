...
class StarService
{
    ...
    {
        if (!$this->elasticsearch->client()->indices()->exists(['index' => 'stars'])->asBool()) {
            return ['total' => 0, 'data' => []];
        }
    ...
    public function create(array $data, string $employeeId): array
    {
        ...
        $result = $this->elasticsearch->client()->index([
            'index' => 'stars', 'body' => $document, 'refresh' => 'wait_for',
        ])->asArray();
    ...
    public function delete(string $id, string $employeeId): bool
    {
        try {
            ...
            $this->elasticsearch->client()->delete(['index' => 'stars', 'id' => $id, 'refresh' => 'wait_for']);
            return true;
